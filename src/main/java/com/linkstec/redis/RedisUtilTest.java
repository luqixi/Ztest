...
//			testmap.put("test2", objectMapper.writeValueAsString(null));
			...
			RedisUtilTest.set("jsonstr", custCache);
			System.out.println(RedisUtilTest.get("jsonstr", Map.class));
			...
			switch (args[0]){
			case "-get" :
			System.out.println("get " + args[1]+"的值是："+RedisUtilTest.get(args[1], String.class));
			    break;
			case "-hget" :
				System.out.println("hget " + args[1] + " " + args[2]+"的值是："+RedisUtilTest.hget(args[1], args[2]));
				break;
			case "-smembers" :
				System.out.println("smembers " + args[1] + " " + "的值是："+RedisUtilTest.sMembers(args[1]));
				break;
			}
			System.out.println("-------------------------华丽的分割线----------------------");
			/*JedisCluster jedisCluster =  getJedisCluster();
